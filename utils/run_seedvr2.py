...
def main():
    parser = argparse.ArgumentParser(description="Run SeedVR2 Upscale Workflow")
    parser.add_argument("--server", default="http://127.0.0.1:8188", help="ComfyUI server address")
    parser.add_argument("--input", required=True, help="Path to input image")
    parser.add_argument("--model", default="seedvr2_ema_3b-Q4_K_M.gguf", help="DiT Model name")
    parser.add_argument("--resolution", type=int, default=1080, help="Output resolution (height)")
    ...
    upload_res = client.upload_image(args.input, overwrite=True)
    uploaded_filename = upload_res['name']
    
    # 2. Modify Workflow
    # Find nodes by ID or Type
    # Node 27: SeedVR2LoadDiTModel (Model)
    # Node 28: SeedVR2VideoUpscaler (Resolution)
    # Node 12: LoadImage (Input Image)
    ...
    for node in workflow['nodes']:
        if node['type'] == 'LoadImage':
            if 'outputs' in node and len(node['outputs']) > 0:
                # Assuming first output is IMAGE and has links
                # We need a valid link ID that traces back to this node.
                # In the JSON, links are [16, 24]. 
                # If we use an existing link ID, convert_ui_to_api will resolve it to this node.
                # We can grab the first link ID.
    ...
    print(f"Removed nodes {nodes_to_remove} to disable low-res preview.")

    for node in workflow['nodes']:
        if node['id'] == 27: # SeedVR2LoadDiTModel
            # Widget 0 is model name
            print(f"Setting model to: {args.model}")
            node['widgets_values'][0] = args.model
            ...
            # The values are [seed, control, resolution, ...]
            # We want [seed, resolution, ...] for the API mapping
            if len(node['widgets_values']) > 1 and isinstance(node['widgets_values'][1], str) and node['widgets_values'][1] in ["fixed", "increment", "decrement", "randomize"]:
                 print(f"Removing control_after_generate widget value: {node['widgets_values'][1]}")
                 node['widgets_values'].pop(1)
            
            # After pop, resolution is at index 1
            print(f"Setting resolution to: {args.resolution}")
            # Ensure we are setting the correct index. 
            # Original: seed, control, resolution(2)
            # New: seed, resolution(1)
            node['widgets_values'][1] = args.resolution
            
            # Bypass Node 24 (ImageScaleToTotalPixels)
            # Connect 'image' input directly to LoadImage output
            if load_image_link is not None:
                print(f"Bypassing scaling node. Connecting SeedVR2VideoUpscaler input to LoadImage (Link {load_image_link})")
                for input_conf in node['inputs']:
                    if input_conf['name'] == 'image':
                        input_conf['link'] = load_image_link
                        break
            
        elif node['type'] == 'LoadImage': # Find LoadImage dynamically if ID changes, or assume ID 12
            # Widget 0 is image name
            print(f"Setting input image to: {uploaded_filename}")
            # LoadImage usually has [filename, prefix]
    ...
    # 3. Convert to API format and Run
    print("Converting workflow to API format...")
    try:
        prompt = client.convert_ui_to_api(workflow)
    except Exception as e:
        print(f"Error converting workflow: {e}")
        # Fallback: dump workflow for debugging
        ...
        for node_id, node_output in outputs.items():
            if 'images' in node_output:
                for image in node_output['images']:
                    ...
                    img_data = client.get_image(filename, subfolder, img_type)
